...
function Vendeur() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const datas = result.category.map((element: { _id: string; slug: string }) => ({
        value: element._id,
        label: element.slug,
      }));
      setOption(datas);
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!boutik.name || !boutik.adresse || !boutik.phoneNumber || !boutik.email) {
    ...
    try {
      if(csrf){
        const response = await fetch(`${import.meta.env.REACT_API_URL}boutiks/store`, {
          method: "POST",
          headers: {
            "xsrf-token": csrf,
          },
  ...
  const handleSelectChange = (selected: MultiValue<CategoryOption>) => {
    setBoutik((prev) => ({ ...prev, product_category: selected as CategoryOption[] }));
  };
    ...
    <form onSubmit={handleSubmit}>
      ...
      <section>
        <div className="container mx-auto px-3">
          <h2 className="text-xl font-semibold my-5">Informations de la boutique</h2>
